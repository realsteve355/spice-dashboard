...
runPage(d => [
  ...
  renderAxionBasketStat(d.axion_basket_trajectory),
  renderBasketComposition(d.categories, d.basket_weights, d.basket_trajectory),
  renderCategoryChart(d.categories, d.basket_trajectory, d.axion_basket_trajectory),
...
function renderAxionBasketStat(traj) {
  ...
  <div class="card" style="margin-bottom:14px; border-left: 3px solid var(--crit);">
    <h3>AXION basket — planning projection (more aggressive than research aggregate)</h3>
    <div class="stats" style="grid-template-columns: repeat(${traj.length}, 1fr);">
      ${traj.map(stat).join('')}
    </div>
    <div style="font-size:11px; color:var(--faint); margin-top:8px;">
      AXION colonies plan against this more aggressive deflation curve: ~95% basket cost reduction by 2045 (vs ~85% in the research aggregate above).
      Used as the design case for AXION architecture. The forecast charts below show both lines.
    </div>
  </div>`;
}
