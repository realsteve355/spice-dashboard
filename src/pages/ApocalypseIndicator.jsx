...
// ─── Collision alignment data ─────────────────────────────────────────────────
const SCENARIO_TRACK = [
  { label: "CBO",      sub: "5% displaced"  },
  { label: "IMF/GS",   sub: "10% displaced" },
  { label: "McKinsey", sub: "25% displaced" },
  { label: "SPICE",    sub: "40% displaced" },
  { label: "Tsunami",  sub: "60% displaced" },
];

const SCENARIO_CONTEXT = [
  "Real-world conditions remain within CBO baseline parameters. Structural debt fragility exists but has not yet manifested in market pricing. The collision is a future risk, not a present one.",
  "Early signals developing. Market indicators beginning to reflect the structural pressures modelled under IMF/GS scenario assumptions. The collision thesis is gaining real-world evidence.",
  "Structural stress accumulating across multiple domains. Conditions broadly consistent with the McKinsey 25% displacement scenario. Collision within a decade on current trajectory.",
  "SPICE base case conditions met. The simulation's 40% displacement scenario is consistent with current market pricing. Without significant policy intervention, collision likely within 3–7 years.",
  "Acute stress across nearly all indicators. Real-world conditions tracking the Tsunami scenario. The simulation's sovereign break triggers are within range.",
];

// Three metrics that directly bridge real-world data to simulation parameters
const BENCHMARKS = [
  {
    label: "Debt / GDP",
    stateKey: "debt_gdp",
    simStart: 123,
    crisisAt: 175,
    crisisNote: "simulation break trigger",
    fmt: v => v.toFixed(1) + "%",
  },
  {
    label: "M2 Growth YoY",
    ...
    simStart: 3,
    crisisAt: 15,
    crisisNote: "QE / debasement territory",
    fmt: v => v.toFixed(1) + "%",
  },
  {
    ...
    simStart: 300,
    crisisAt: 1000,
    crisisNote: "2020 COVID peak: ~1100 bps",
    fmt: v => Math.round(v) + " bps",
  },
];

// ─── Collision alignment panel ─────────────────────────────────────────────────
function CollisionAlignment({ totalScore, states }) {
  const cl = compositeLevel(totalScore);
  const lm = LEVEL_META[cl];

  return (
    <div style={SA.wrap}>
      <div style={SA.eyebrow}>COLLISION MODEL ALIGNMENT</div>
      <div style={SA.sub}>
        Current real-world conditions mapped against the five SPICE simulation scenarios
      </div>

      {/* Scenario track — five boxes, current highlighted */}
      <div style={SA.track}>
        {SCENARIO_TRACK.map((s, i) => (
          <div
            key={i}
            style={{
              ...SA.trackItem,
              background: i === cl ? lm.color : "#f5f5f5",
              color: i === cl ? "#fff" : "#aaa",
              borderColor: i === cl ? lm.color : "#e8e8e8",
            }}
          >
            <div style={SA.trackLabel}>{s.label}</div>
            <div style={SA.trackSub}>{s.sub}</div>
          </div>
        ))}
      </div>

      {/* Scenario narrative */}
      <div style={SA.context}>{SCENARIO_CONTEXT[cl]}</div>

      {/* Benchmark rows */}
      <div style={SA.tableHead}>
        <span style={{ flex: "0 0 160px" }}>Metric</span>
        <span style={{ flex: "0 0 90px", textAlign: "right" }}>Current</span>
        <span style={{ flex: 1, paddingLeft: 20 }}>Progress toward crisis</span>
        <span style={{ flex: "0 0 220px", textAlign: "right" }}>Crisis level</span>
      </div>

      {BENCHMARKS.map(b => {
        const st = states[b.stateKey];
        const val = st?.status === "ok" ? st.value : null;
        const pct = val != null
          ? Math.max(0, Math.min(100, ((val - b.simStart) / (b.crisisAt - b.simStart)) * 100))
          : null;

        return (
          <div key={b.label} style={SA.row}>
            <span style={{ flex: "0 0 160px", fontSize: 12, color: "#333" }}>{b.label}</span>
            <span style={{ flex: "0 0 90px", textAlign: "right", fontSize: 14, fontWeight: 700, color: val != null ? lm.color : "#ccc" }}>
              {val != null ? b.fmt(val) : "—"}
            </span>
            <div style={{ flex: 1, paddingLeft: 20, display: "flex", alignItems: "center", gap: 10 }}>
              <div style={SA.barTrack}>
                <div style={{ ...SA.barFill, width: pct != null ? `${pct}%` : "0%", background: lm.color }} />
              </div>
              <span style={SA.barLabel}>
                {pct != null ? pct.toFixed(0) + "% of the way" : "loading"}
              </span>
            </div>
            <span style={{ flex: "0 0 220px", textAlign: "right", fontSize: 10, color: "#aaa" }}>
              {b.crisisNote}
            </span>
          </div>
        );
      ...
      <div style={SA.linkRow}>
        <a href="/simulation" style={SA.link}>→ Run the collision simulation</a>
      </div>
    </div>
  );
}
...
export default function ApocalypseIndicator() {
  ...
  return (
    <div style={S.page}>
      ...
      {/* Collision model alignment */}
      <CollisionAlignment totalScore={totalScore} states={states} />
