...
export default function Company() {
  ...
  async function handleBuyback(assetId) {
    ...
    try {
      ...
      await tx.wait()
      setActDone(`Bought back ${bps} bps for <MondSymbol size={12} /> {price} — shares cancelled`)
            ...
            {companyAssets.map((a, i) => (
              <div key={a.id} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                  ...
                  <div style={{ fontSize: 12, color: C.text }}><MondSymbol size={12} /> {a.value}</div>
                </div>
          ...
          <div style={card}>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
              <div style={{ position: 'relative', width: 100, flexShrink: 0 }}>
                ...
                />
                <span style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', fontSize: 11, color: C.faint }}><MondSymbol size={10} /></span>
              </div>
              <input
                    ...
                    <div style={{ marginTop: 6, marginLeft: 18 }}>
                      ...
                      {buybackForId === e.assetId && (
                        <div style={{ marginTop: 8, padding: 10, background: `${C.text}08`, border: `1px solid ${C.border}`, borderRadius: 0 }}>
                          ...
                          >
                            {actionPending ? 'Buying back…' : `Pay <MondSymbol size={12} /> {buybackPriceS || '0'} → cancel ${buybackBps || '0'} bps`}
                          </button>
                        </div>
                      )}
                    </div>
                  ...
                  return (
                    <div key={inv.id} style={{
                      ...
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0, marginLeft: 8 }}>
                        <div style={{ fontSize: 13, fontWeight: 500, color: C.text }}><MondSymbol size={12} /> {inv.amount}</div>
                        {isBuyer && isSecretary && (
                          <button
            ...
            {paid.length > 0 && (
              <div style={{ marginBottom: 10 }}>
                <div style={{ fontSize: 10, color: C.faint, letterSpacing: '0.08em', marginBottom: 6 }}>
                  DELIVERY HISTORY · <MondSymbol size={12} /> {paid.reduce((s, i) => s + i.amount, 0)} paid
                </div>
                {paid.map(inv => (
                  <div key={inv.id} style={{
                    display: 'flex', justifyContent: 'space-between', alignItems: 'center',
            ...
            return (
              <div key={contract.id} style={{ ...card, opacity: 0.7 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  ...
                  {total > 0 && <div style={{ fontSize: 13, fontWeight: 500, color: C.faint, flexShrink: 0, marginLeft: 12 }}><MondSymbol size={12} /> {total}</div>}
                </div>
              </div>
            )
          })}
