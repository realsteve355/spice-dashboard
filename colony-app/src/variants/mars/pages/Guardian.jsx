// variants/mars/pages/Guardian.jsx
// Mars Guardian — V-pool-for-minors flow. Children registered from birth
// receive UBI and accumulate V; the pool transfers to the citizen at 18
// when they sign the constitution. Mars-specific.
          ...
          <div>
            ...
            <div style={card}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: child.mccBill.breakdown.length > 0 ? 10 : 0 }}>
                ...
                <span style={{ fontSize: 13, fontWeight: 500, color: C.red, display: 'inline-flex', alignItems: 'baseline', gap: 3 }}>
                  <MondSymbol size={12} />
                  {child.mccBill.total}
                </span>
              </div>
              {child.mccBill.breakdown.map((b, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: C.sub }}>
                  <span>{b.service}</span>
                  <span style={{ color: C.red, display: 'inline-flex', alignItems: 'baseline', gap: 3 }}>
                    <MondSymbol size={11} />
                    {b.amount}
                  </span>
                </div>
              ...
              {child.transactions.length === 0 ? (
                ...
                child.transactions.map((tx, i) => (
                  <div key={i} style={{
                    ...
                    <div style={{ fontSize: 13, fontWeight: 500, color: tx.amount > 0 ? C.green : C.red, display: 'inline-flex', alignItems: 'baseline', gap: 3 }}>
                      {tx.amount > 0 ? '+' : ''}<MondSymbol size={11} />{Math.abs(tx.amount)}
                    </div>
                  </div>
                ))
              )}
